...
hl.bind(mainMod .. " + SPACE",           hl.dsp.exec_cmd("rofi -show drun -theme /home/ridu/.config/rofi/config.rasi"))
hl.bind(mainMod .. " + V",               hl.dsp.exec_cmd('bash -c "cliphist list | rofi -dmenu -i -theme ~/.config/rofi/config.rasi | cliphist decode | wl-copy"'))
...
hl.bind(mainMod .. " + SHIFT + RETURN",  hl.dsp.exec_cmd("code"))

-- Special Workspace (scratchpad terminal)
-- hl.bind(mainMod .. " + grave", hl.dsp.workspace.toggle_special("term"))

-- Logout / power
hl.bind(mainMod .. " + ESCape", hl.dsp.exec_cmd("bash ~/.config/rofi/powermenu.sh"))
...
-- === Fn / Media Keys ===
hl.bind("XF86AudioRaiseVolume",  hl.dsp.exec_cmd("swayosd-client --max-volume 153 --output-volume raise"), { locked = true, repeating = true })
hl.bind("XF86AudioLowerVolume",  hl.dsp.exec_cmd("swayosd-client --output-volume lower"),                  { locked = true, repeating = true })
hl.bind("XF86AudioMute",         hl.dsp.exec_cmd("swayosd-client --output-volume mute-toggle"),            { locked = true })
hl.bind("XF86MonBrightnessUp",   hl.dsp.exec_cmd("swayosd-client --brightness raise"),                     { locked = true, repeating = true })
hl.bind("XF86MonBrightnessDown", hl.dsp.exec_cmd("swayosd-client --brightness lower"),                     { locked = true, repeating = true })
hl.bind("XF86AudioMicMute",      hl.dsp.exec_cmd('bash -c \'wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle && if wpctl get-volume @DEFAULT_AUDIO_SOURCE@ | grep -q MUTED; then notify-send "Mic Muted"; else notify-send "Mic Unmuted"; fi\''), { locked = true })
hl.bind("XF86RFKill",            hl.dsp.exec_cmd('bash -c \'STATE=$(nmcli radio wifi); if [ "$STATE" = "enabled" ]; then nmcli radio wifi off && notify-send "Airplane Mode ON"; else nmcli radio wifi on && notify-send "Airoplane Mode OFF"; fi\''))
...
hl.bind(mainMod .. " + code:20",
    hl.dsp.window.resize({ x = -100, y = 0, relative = true }))

hl.bind(mainMod .. " + code:21",
    hl.dsp.window.resize({ x = 100, y = 0, relative = true }))
...
-- === Screenshots ===
hl.bind("Print",         hl.dsp.exec_cmd('bash -c \'grim -g "$(slurp)" - | tee ~/Pictures/Screenshots/Screenshot-$(date +%F_%T).png | wl-copy --type image/png && notify-send "Screenshot copied + saved"\''))
hl.bind("SHIFT + Print", hl.dsp.exec_cmd('bash -c \'grim - | tee ~/Pictures/Screenshots/Screenshot-$(date +%F_%T).png | wl-copy --type image/png && notify-send "Copied + saved screenshot"\''))
